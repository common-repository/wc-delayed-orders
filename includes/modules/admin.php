...
class Admin {
	...
	public function register_hooks() {
		...
		if ( $this->instance->is_active ) {
			...
			add_action( 'restrict_manage_posts', array( $this, 'add_filter_input' ) );
			add_action( 'manage_edit-shop_order_columns', array( $this, 'add_custom_column_header' ), 20 );
			add_action( 'manage_shop_order_posts_custom_column', array( $this, 'add_custom_column_data' ), 20, 2 );
			add_action( 'save_post', array( $this, 'update_delay_status' ), 10, 3 );
			add_action( 'admin_init', array( $this, 'update_delay_status_all' ) );
			add_action( 'pre_get_posts', array( $this, 'posts_query_mod' ) );
			add_action( 'admin_bar_menu', array( $this, 'admin_toolbar_delayed_orders' ), 100 );

			$unmarked = wcdo_count_unmarked_orders( $wcdo_settings['delay_order_statuses'] );
			if ( $unmarked > 0 ) {
				add_action( 'admin_notices', array( $this, 'unmarked_orders_notification' ) );
			}
		} else {

			add_action( 'admin_notices', array( $this, 'requirements_notification' ) );
	...
	public function requirements_notification() { ?>
		...
		<div class="notice notice-error">
			<p>
				...
				<?php _e( 'Please install and activate', 'wc-delayed-orders' ); ?> <a href="<?php echo admin_url( '/plugin-install.php?s=WooCommerce&tab=search&type=term' ); ?>"><?php _e( 'WooCommerce', 'wc-delayed-orders' ); ?></a>.
			</p>
		</div>
	...
	public function unmarked_orders_notification() {
		...
		<div class="notice notice-warning">
			...
			<p><?php _e( 'We found', 'wc-delayed-orders' ); ?> <strong><?php echo $unmarked; ?> <?php _e( 'order(s)', 'wc-delayed-orders' ); ?></strong> <?php _e( 'without a delay status. It is recommended to update all orders.', 'wc-delayed-orders' ); ?></p>
		</div>
	...
	public function add_filter_input() {
		...
		if ( $post_type == 'shop_order' ) {
			...
			<input type="text" name="delay_days" value="<?php echo esc_html( $value ); ?>" placeholder="<?php echo $placeholder; ?>" style="width: 240px;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
	...
	public function add_custom_column_data( $column, $post_id ) {
		...
		if ( $column == 'delay-status' && in_array( get_post_status( $post_ID ), $wcdo_settings['delay_order_statuses'] ) ) {

			$order      = wc_get_order( $post_id );
			$order_date = $order->get_date_created();
			$delay      = human_time_diff( strtotime( $order_date ), time() );
			$icon       = $delay_status != '';
			$class      = array( '', ' green', ' red' )[ $icon ? ( intval( $delay_status ) + 1 ) : 0 ];

			?>

				<mark class="delayed-order-tag<?php echo $class; ?>">
					<span>
						<?php if ( $icon ) : ?>
	...
	public function update_delay_status( $post_ID, $post, $update ) {
		...
		if ( in_array( get_post_status( $post_ID ), $wcdo_settings['delay_order_statuses'] ) ) {
			...
			$range = array( 'days', 'weeks', 'months', 'years' )[ $wcdo_settings['delay_days_range'] ];
	...
	public function update_delay_status_all() {
		...
		$query = new \WP_Query(
			array(
				...
				'posts_per_page' => -1,
				'post_status'    => $wcdo_settings['delay_order_statuses'],
			)
		);
	...
	public function posts_query_mod( $wp_query ) {
		...
		if ( is_admin() && $pagenow == 'edit.php' ) {
			...
			add_filter( 'views_edit-shop_order', array( $this, 'add_filter_link' ) );
			...
			if ( $wp_query->query_vars['post_type'] == 'shop_order' ) {
				...
				if ( ( isset( $_GET['delay_status'] ) && $_GET['delay_status'] == '1' ) || isset( $_GET['delay_days'] ) ) {
					...
					$wp_query->set( 'post_status',  $wcdo_settings['delay_order_statuses']);
